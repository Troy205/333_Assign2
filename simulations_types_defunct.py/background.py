...
def main(
    ...
    radial_z=0.5,
    radial_scale=0.01,
    sky_rgb = np.array([0.45, 0.65, 1.0]),  # light blue, sRGB
    alpha = 0.6
):
    ...
    # --- Convert to displayable color (linear -> sRGB)
    display_img = linear_image_to_srgb_image(image, clip_percentile=100)

    # --- Blend with sky background
    display_img = (1 - alpha) * sky_rgb + alpha * display_img
    display_img = np.clip(display_img, 0, 1)

    # --- Show the blended image
    plt.figure(figsize=(6, 6))
    plt.imshow(display_img, origin="lower", extent=[-1, 1, -1, 1])
    plt.axis("off")
    plt.title("Simulated birefringence over sky background")
    plt.show()
